...
const gameLoop = {
  init() {
    ...
    this.physicsManager.createPhysicsWorld().then(world => {
      this.physicsManager.createCharacter(playerManager.players.get(0));
  ...
  update() {
    inputManager.handleInput(this.physicsManager.player);
  ...
  checkCollisions(player) {
    ...
    worldManager.platforms.forEach((platform) => {
      ...
      if (platform.groundPoints) {
        ...
        const groundHeight = worldManager.getGroundHeightAt(
          player.position.x
        );
